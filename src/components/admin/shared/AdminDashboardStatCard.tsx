...
export const AdminDashboardStatCard: React.FC<AdminDashboardStatCardProps> = ({
  ...
  return (
    <div
      className={cn(
        "bg-white border border-[#C4A482]/20 rounded-2xl p-6 shadow-sm flex flex-col justify-between gap-4 hover:border-brand-primary/30 transition-all duration-300 text-left font-poppins",
        className
      )}
    >
      <div className="flex items-center justify-between">
        <div className={cn("p-2.5 rounded-xl", getKpiIconBg(iconType))}>{getKpiIcon(iconType)}</div>

        <div
          className={cn(
            "flex items-center gap-0.5 px-2.5 py-1 rounded-full text-[10px] font-bold",
            isUp ? "bg-[#4A5E28]/10 text-brand-green" : "bg-red-500/10 text-red-500"
          )}
        >
      ...
      <div className="space-y-1">
        <span className="text-2xl font-bold font-poppins text-[#3A2418]">{value}</span>
        <h4 className="text-xs font-semibold text-[#8D7F75] tracking-wider uppercase block">
          {title}
        </h4>
      </div>

      <div className="border-t border-gray-100 pt-3 text-[11px] text-[#8D7F75]">{subtext}</div>
    </div>
  );
};
